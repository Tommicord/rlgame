...
static int
R_Pack_LoadImgAsset (const char* pPath, struct R_Pack_InputImage* pImage, uint8_t** ppPixelBuffer)
{
...
static struct R_Pack_Encoder*
R_Pack_CreateEncoder (const struct R_Pack_EncoderConfig* pConfig)
{
    struct R_Pack_Encoder* pEncoder = R_Pack_NewEncoder (pConfig);
    if (!pEncoder)
    {
...
static uint32_t R_Pack_EncodeInputImagesParallel (
    struct R_Pack_Encoder*     pEncoder,
    const struct R_CSTL_Array* pInputPaths,
    uint32_t                   mipmapSize,
    ...
    const char*                        pOutputPath)
{
    ...
    for (size_t i = 0; i < sizeof (mipmapSizes) / sizeof (mipmapSizes[0]); ++i)
    {
        ...
        struct R_Pack_Encoder* pVariantEncoder = R_Pack_CreateEncoder (pConfig);
        if (!pVariantEncoder)
        {
            R_CSTL_LOG_ERROR (
        ...
        uint32_t successCount = R_Pack_EncodeInputImagesParallel (
            pVariantEncoder,
            pInputPaths,
            mipmapSizes[i],
            pConfig->workerCount);
...
R_Pack_WorkerThreadFunc (void* pData)
{
    ...
    while (1)
    {
        ...
        int loadResult = R_Pack_LoadImgAsset (task.pPath, task.pImage, task.ppPixelBuffer);
...
static uint32_t
R_Pack_EncodeInputImagesParallel (
    struct R_Pack_Encoder*     pEncoder,
    const struct R_CSTL_Array* pInputPaths,
    uint32_t                   mipmapSize,
    uint32_t                   workerCount)
...
static void
R_Pack_CleanupResources (struct R_Pack_Encoder* pEncoder, struct R_CSTL_Array* pInputPaths)
{
    if (pEncoder)
    {
...
main (int argc, char** argv)
{
    ...
    uint32_t successCount = R_Pack_EncodeInputImagesParallel (pEncoder, pInputPaths, 0, config.workerCount);
    if (successCount == 0)
    {

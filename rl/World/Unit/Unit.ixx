...
{
protected:
    /* Stores the textures as a union */
    union Material
    {
        struct Faces
        {
    ...
    /* Describes if it is using a static stack-allocated textures or heap-allocated */
    bool heap = false;
...
protected:
    ...
    inline static std::vector<std::string_view> defaultName = {
        std::string_view("rl.unit")};
...
protected:
    static void InitGPURegistry()
    {
        ...
        if (!gpuRegistry->IsInitialized())
        {
            const Main::MainBinding binding = Main::Game::GetInstance().GetMainBinding();
            gpuRegistry->Initialize(binding.device, binding.physicalDevice);
        }
    }

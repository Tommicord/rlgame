...
std::vector<float> UnitDynamicTexture::GenNoiseValMap(const float scale) const
{
    ...
    for (; i + simdWidth <= totalPixels; i += simdWidth)
    {
        ...
        float32x4_t indices = vsetq_lane_f32(
            i + 3,
            vsetq_lane_f32(
                i + 2, vsetq_lane_f32(i + 1, vsetq_lane_f32(i, vdupq_n_f32(0), 0), 1), 2),
            3);
...
Texture2* UnitDynamicTexture::GenTexture(uint8_t*     data,
                                         const size_t width,
                                         const size_t height,
                                         const size_t channels)
{
    ...
    newTexture->FromData(data, static_cast<int>(width), static_cast<int>(height),
                         (channels == 4) ? Texture2Format::RGBA8 : Texture2Format::RGB8,
                         Texture2Properties());
    delete[] data;
    return newTexture;
}
...
uint8_t UnitDynamicTexture::ClampNoiseVar(const int v, const float variation) const
{
    ...
    const uint8_t res =
        static_cast<uint8_t>(std::clamp(vFloat + variation, 0.0f, 255.0f));
    return res;
}

void UnitDynamicTexture::ApplyNoiseVar(uint8_t&                  r,
                                       uint8_t&                  g,
                                       uint8_t&                  b,
                                       int                       index,
                                       const std::vector<float>& noiseMap) const
{
    ...
    if (index + 3 < noiseMap.size())
    {
        ...
        // Apply to RGB values
        float32x4_t rgb = vsetq_lane_f32(
            r, vsetq_lane_f32(g, vsetq_lane_f32(b, vdupq_n_f32(0), 0), 1), 2);
    ...
    uint8_t& r, uint8_t& g, uint8_t& b, int index, const std::vector<int>& colorMap) const
{
    ...
    if (index < colorMap.size())
    {
        ...
        float32x4_t base          = vsetq_lane_f32(
            r, vsetq_lane_f32(g, vsetq_lane_f32(b, vdupq_n_f32(0), 0), 1), 2);
        float32x4_t palette = vsetq_lane_f32(
            pr, vsetq_lane_f32(pg, vsetq_lane_f32(pb, vdupq_n_f32(0), 0), 1), 2);

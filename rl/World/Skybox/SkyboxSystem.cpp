...
glm::vec3 SkyboxSystem::InterpolateSkyColor(
  ...
  case Time::TimeOfDay::DAWN:
    {
      ...
      return InterpolateSkyColor(
          currentPreset.nightSkyColor, currentPreset.dawnSkyColor, dawnProgress);
    }
  case Time::TimeOfDay::DAY:
    {
      ...
      if (dayProgress < 0.5f)
      {
        ...
        return InterpolateSkyColor(
            currentPreset.dawnSkyColor, currentPreset.daySkyColor, t);
      }
      else
      {
        // Noon to dusk
        ...
        return InterpolateSkyColor(
            currentPreset.daySkyColor, currentPreset.duskSkyColor, t);
      }
    }
  case Time::TimeOfDay::DUSK:
    {
      ...
      float duskProgress = static_cast<float>(fragment - duskStart) /
                           static_cast<float>(Time::DAY_DURATION_FRAGMENTS * 0.1f);
      return InterpolateSkyColor(
          currentPreset.duskSkyColor, currentPreset.nightSkyColor, duskProgress);
    }
...
glm::vec3 SkyboxSystem::GenerateColorVariation(
    const glm::vec3& baseColor, float variationAmount)
{

...
/* Production-ready GPU biome registry for compute shader access */
export class BiomeRegistryGPU
{
  ...
  [[nodiscard]]
  VkBuffer GetBiomeBuffer() const { return biomeBuffer; }

  /* Get the GPU buffer handle for unit rules */
  [[nodiscard]]
  VkBuffer GetUnitRulesBuffer() const { return unitRulesBuffer; }

  /* Get the total number of registered biomes */
  [[nodiscard]]
  uint32_t GetBiomeCount() const { return static_cast<uint32_t>(cpuBiomes.size()); }

  /* Get the total number of unit rules across all biomes */
  [[nodiscard]]
  uint32_t GetUnitRuleCount() const { return static_cast<uint32_t>(cpuUnitRules.size()); }
  ...
  bool CopyBufferToGPU(VkDevice device, VkCommandBuffer commandBuffer,

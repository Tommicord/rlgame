...
ChunkInRenderUnits::ChunkInRenderUnits(
    int32_t renderDistanceX, int32_t renderDistanceY, int32_t renderDistanceZ) :
...
bool ChunkInRenderUnits::AddChunk(
    const WorldChunkCoord& coord, UnitChunkBuffer& chunkBuffer)
{
  if (!initialized.load())
    return false;
...
TransactionResult ChunkInRenderUnits::ReadUnitId(
    const WorldChunkCoord& coord, int32_t localX, int32_t localY, int32_t localZ)
{
  if (!initialized.load())
  ...
  if (!IsInChunkBounds(localX, localY, localZ))
    return TransactionResult::Error(
        "Local coordinates out of bounds", ValidationResult::INVALID_COORDINATES);

  UnitChunkBuffer* chunk = GetChunkBuffer(coord);
  if (!chunk)
    return TransactionResult::Error(
        "Chunk not found", ValidationResult::INVALID_CHUNK_INDEX);
    ...
    uint32_t                                                             newUnitId)
{
  ...
  if (!oldUnitId)
    return TransactionResult::Error(
        "Failed to read current unit ID", ValidationResult::VALID);
...
bool ChunkInRenderUnits::IsInChunkBounds(

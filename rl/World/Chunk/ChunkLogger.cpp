    ...
    logEntries(std::move(other.logEntries)), enabled(other.enabled.load())
{
    for (uint32_t i = 0; i < 6; ++i)
    {
        logCounts[i].store(other.logCounts[i].load(std::memory_order_acquire),
...
void ChunkLogger::Log(RayLog::RayLogLevel level,
                           ...
                           int32_t             localZ)
{
    if (!enabled.load(std::memory_order_acquire))
        return;
    const std::string chunkMsg = std::format("[Chunk {} @({}, {}, {})] %s", chunkIndex,
                                             localX, localY, localZ, message);
    switch (level)
    {
    case RayLog::RayLogLevel::Trace:

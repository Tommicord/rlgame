                                 ...
                                 uint32_t         seed)
{
  ...
  {
    throw std::runtime_error("No graphics queue family is available for permutation uploads");
  }
      ...
      VK_SUCCESS)
  {
    ...
    throw std::runtime_error("Failed to create temporary command pool for permutation upload");
  }
  ...
  if (queue == VK_NULL_HANDLE)
  {
    ...
    vkDestroyBuffer(device, stagingPermGradBuffer, nullptr);
    vkFreeMemory(device, stagingPermGradBufferMemory, nullptr);
    throw std::runtime_error("No graphics queue is available for permutation uploads");
  }

...
UnitChunkAccessor::UnitChunkAccessor(ChunkInRenderUnits& chunkSystem, const UnitPosition& unitPosition)
...
TransactionResult UnitChunkAccessor::ReadRelative(const RelativeOffset& offset)
{
  ...
  {
    return TransactionResult::Error("Relative position is out of bounds", ValidationResult::INVALID_COORDINATES);
  }
  ...
  {
    return TransactionResult::Error("Failed to convert world position to chunk coordinates", ValidationResult::VALID);
  }
  
  return chunkSystem.ReadUnitId(chunkCoord, localX, localY, localZ);
}

TransactionResult UnitChunkAccessor::WriteRelative(const RelativeOffset& offset, uint32_t newUnitId)
{
  UnitPosition absolutePos = GetAbsolutePosition(offset);
...
bool UnitChunkAccessor::WorldToChunkLocal(const UnitPosition& worldPos, WorldChunkCoord& chunkCoord,

...
UnitChunkAccessor::UnitChunkAccessor(ChunkInRenderUnits& chunkSystem,
                                     const UnitPosition& unitPosition) :
...
TransactionResult UnitChunkAccessor::ReadRelative(const RelativeOffset& offset)
{
    ...
    {
        return TransactionResult::Error("Relative position is out of bounds",
    ...
    if (!WorldToChunkLocal(absolutePos, chunkCoord, localX, localY, localZ))
    {
        return TransactionResult::Error(
            "Failed to convert world position to chunk coordinates",
...
TransactionResult UnitChunkAccessor::WriteRelative(const RelativeOffset& offset,
                                                   uint32_t              newUnitId)
{
    UnitPosition absolutePos = GetAbsolutePosition(offset);

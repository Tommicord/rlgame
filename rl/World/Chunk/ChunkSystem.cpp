...
void ChunkSystem::RefreshAndWait()
{
    refreshRequested.store(true, std::memory_order_release);
    const auto deadline =
        std::chrono::steady_clock::now() + std::chrono::milliseconds(512);
...
void ChunkSystem::RefreshInternal()
{
    ...
    try
    {
        ...
        std::vector<WorldChunkCoord> coordsToGenerate;
        {
            ...
            for (int32_t z = startZ; z <= endZ; ++z)
            {
                for (int32_t y = startY; y <= endY; ++y)
                {
                    ...
                    {
                        WorldChunkCoord coord{playerChunk.chunkX + x,
                                              playerChunk.chunkY + y,
        ...
        while (pendingChunkIndex < pendingChunks.size() &&
...
uint32_t ChunkSystem::DetermineMaxChunksPerFrame() const
{
    ...
    if (gpuGenerationEnabled && gpuPhysicalDevice != VK_NULL_HANDLE)
    {
        ...
        if (isIntegratedGPU)
        {
            ...
            {
                maxChunks = 1; // Typical integrated (like Intel HD), reduced to 1 to
                               // prevent timeout
                useReducedDimensions = true;
            }
            else
            {
                ...
                useReducedDimensions =
                    true; // Always use reduced dimensions for integrated
            }
        }
        else
        {
        ...
        RayLog::LogInfo(RAYLOG_TAG,
                        "GPU detected: %s, Total Memory: %.2f GB, Max chunks/frame: %u, "
...
bool ChunkSystem::GenerateChunk(const WorldChunkCoord& coord)
{
    ...
    if (useGPU && localGpuGenerator && localGpuDevice != VK_NULL_HANDLE)
    {
        ...
        if (!GenerateChunkWithGpu(localGpuGenerator, localGpuDevice,
                                  localGpuPhysicalDevice, coord, chunkBuffer))
            return false;
    }
    else
    {

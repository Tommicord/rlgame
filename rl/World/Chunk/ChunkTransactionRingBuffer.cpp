...
ChunkTransactionRingBuffer::ChunkTransactionRingBuffer(
    ...
    bufferSize(other.bufferSize), head(other.head.load()), tail(other.tail.load()),
...
bool ChunkTransactionRingBuffer::PushWithTimeout(ChunkTransaction transaction,
...
bool ChunkTransactionRingBuffer::PopWithTimeout(ChunkTransaction& transaction,
                                                const uint32_t    timeoutMs)
{
    const auto startTime       = std::chrono::steady_clock::now();
    const auto timeoutDuration = std::chrono::milliseconds(timeoutMs);
...
RingBufferStats ChunkTransactionRingBuffer::GetStats() const
{
    ...
    {
        stats.averageSize =
            static_cast<double>(peakSize.load(std::memory_order_acquire)) /

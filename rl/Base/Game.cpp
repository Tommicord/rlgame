...
Game::Game() :
    ...
    input(Input::UserInput::GetInstance()), debugMessenger(VK_NULL_HANDLE),
    headless(false)
{
}
...
void Game::DestroyGraphics()
{
    ...
    if (binding.device != VK_NULL_HANDLE)
    {
        ...
        {
            vkDestroyDescriptorSetLayout(binding.device, binding.descriptorSetLayout,
                                         nullptr);
        }
    ...
    if (binding.instance != VK_NULL_HANDLE)
    {
        if (enableValidationLayers && debugMessenger != VK_NULL_HANDLE)
        {
            auto func = (PFN_vkDestroyDebugUtilsMessengerEXT)vkGetInstanceProcAddr(
                binding.instance, "vkDestroyDebugUtilsMessengerEXT");
            if (func != nullptr)
            {
...
void Game::InitWindow()
{
    ...
    window = glfwCreateWindow(Rl::Main::width, Rl::Main::height, "The Real Game", nullptr,
                              nullptr);
    GLFWmonitor*       monitor = glfwGetPrimaryMonitor();
    const GLFWvidmode* mode    = glfwGetVideoMode(monitor);
    glfwSetWindowPos(window, (mode->width - Rl::Main::width) / 2,
                     (mode->height - Rl::Main::height) / 2);
    ...
    glfwSetCursorPosCallback(window,
                             [](GLFWwindow* window, const double xpos, const double ypos)
                             {
                                 ...
                                 glfwSetCursorPos(window, Rl::Main::width / 2.0,
                                                  Rl::Main::height / 2.0);
                                 game.input.NotifyMouseMoveEvent(event);
                             });

    glfwSetKeyCallback(window,
                       [](GLFWwindow* window, const int key, const int scancode,
...
void Game::CreateLogicalDevice()
{
    ...
    if (!headless)
    {
        vkGetDeviceQueue(binding.device, binding.queueFamilyIndices.presentFamily.value(),
                         0, &binding.presentQueue);
    }
}
...
void Game::CreateSwapChain()
{
    ...
    vkGetPhysicalDeviceSurfaceFormatsKHR(binding.physicalDevice, binding.surface,
                                         &formatCount, nullptr);
    ...
    vkGetSwapchainImagesKHR(binding.device, binding.swapChain, &imageCount,
                            binding.swapChainImages.data());
...
void Game::CreateRenderPass()
{
    ...
    if (vkCreateRenderPass(binding.device, &renderPassInfo, nullptr,
                           &binding.renderPass) != VK_SUCCESS)
    {
...
void Game::CreateCommandBuffers()
{
    ...
    if (vkAllocateCommandBuffers(binding.device, &allocInfo,
                                 &binding.commandBuffers[0]) != VK_SUCCESS)
    {
...
void Game::Draw()
{
    ...
    vkCmdBeginRenderPass(binding.commandBuffers[0], &renderPassInfo,
...
MainBinding::QueueFamilyIndices Game::FindQueueFamilies(VkPhysicalDevice device) const
{
    ...
    for (const auto& queueFamily : queueFamilies)
    {
        ...
        if (!headless)
        {
            ...
            vkGetPhysicalDeviceSurfaceSupportKHR(device, i, binding.surface,
                                                 &presentSupport);
            if (presentSupport)
            {

...
bool Texture2::FromMemory(const uint8_t*            data,
                          size_t                    size,
                          const Texture2Properties& properties)
{
    this->properties = properties;
    ...
    stbi_uc* imageData = stbi_load_from_memory(data, static_cast<int>(size), &width,
                                               &height, &channels, 0);
                ...
                for (int x = 0; x < mipWidth; ++x)
                {
                    ...
                    for (int c = 0; c < channels; ++c)
                    {
                        ...
                        for (int dy = 0; dy < 2; ++dy)
                        {
                            ...
                            {
                                int srcX = (x * 2 + dx) < prevWidth ? (x * 2 + dx)
                                                                    : prevWidth - 1;
                                int srcY = (y * 2 + dy) < prevHeight ? (y * 2 + dy)
                                                                     : prevHeight - 1;

                                size_t srcOffset =
                                    (srcY * prevWidth + srcX) * channels + c;

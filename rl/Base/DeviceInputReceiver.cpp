...
void DeviceInputReceiver::PollWindowsInput() {
    ...
    for (int i = 0; i < 256; ++i) {
        ...
        if (isPressed && previousState == 0) {
            ...
            event.modifiers = 0;
            if (GetAsyncKeyState(VK_SHIFT)) event.modifiers |= static_cast<int>(Modifier::Shift);
            if (GetAsyncKeyState(VK_CONTROL)) event.modifiers |= static_cast<int>(Modifier::Control);
            if (GetAsyncKeyState(VK_MENU)) event.modifiers |= static_cast<int>(Modifier::Alt);

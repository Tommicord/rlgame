...
void Texture2::UploadTextureData(Game::VulkanContext& context)
{
  ...
  if (vkCreateBuffer(context.device, &stagingBufferInfo, nullptr, &binding.vkStagingBuffer) !=
      VK_SUCCESS)
  {
  ...
  if (vkAllocateMemory(
          context.device, &stagingAllocInfo, nullptr, &binding.vkStagingBufferMemory) != VK_SUCCESS)
  {
  ...
  if (mipmapLevels > 1)
  {
    ...
    for (uint32_t i = 1; i < static_cast<uint32_t>(mipmapLevels); i++)
    {
      ...
      vkCmdBlitImage(commandBuffer, binding.vkImage, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL,
          binding.vkImage, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &blit, VK_FILTER_LINEAR);

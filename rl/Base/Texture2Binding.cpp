...
void Texture2::CreateBindingImage(Main::MainBinding& context)
{
    ...
    if (vkCreateImage(context.device, &imageInfo, nullptr, &binding.vkImage) !=
        VK_SUCCESS)
    {
...
void Texture2::UploadTextureData(Main::MainBinding& context)
{
    ...
    if (vkCreateBuffer(context.device, &stagingBufferInfo, nullptr,
                       &binding.vkStagingBuffer) != VK_SUCCESS)
    {
        throw std::runtime_error("Failed to create staging buffer");
    }

    VkMemoryRequirements stagingMemRequirements;
    vkGetBufferMemoryRequirements(context.device, binding.vkStagingBuffer,
    ...
    vkBindBufferMemory(context.device, binding.vkStagingBuffer,
                       binding.vkStagingBufferMemory, 0);

    // Copy texture data to staging buffer
    void* mappedData;
    vkMapMemory(context.device, binding.vkStagingBufferMemory, 0, dataSize, 0,
                &mappedData);
    ...
    vkCmdCopyBufferToImage(commandBuffer, binding.vkStagingBuffer, binding.vkImage,
    ...
    if (mipmapLevels > 1)
    {
        ...
        for (uint32_t i = 1; i < static_cast<uint32_t>(mipmapLevels); i++)
        {
            ...
            vkCmdBlitImage(commandBuffer, binding.vkImage,
                           VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, binding.vkImage,
        ...
        mipBarrier.subresourceRange.baseMipLevel =
            static_cast<uint32_t>(mipmapLevels) - 1;
        mipBarrier.subresourceRange.levelCount = 1;
        mipBarrier.oldLayout                   = VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL;
        mipBarrier.newLayout                   = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
        mipBarrier.srcAccessMask               = VK_ACCESS_TRANSFER_WRITE_BIT;
    ...
    if (vkQueueSubmit(context.graphicsQueue, 1, &submitInfo, VK_NULL_HANDLE) !=
        VK_SUCCESS)
    {
...
void Texture2::CreateBindingSampler(Main::MainBinding& context)
{
    ...
    if (vkCreateSampler(context.device, &samplerInfo, nullptr, &binding.vkSampler) !=
        VK_SUCCESS)
    {
...
void Texture2::GetImageView(Main::MainBinding& context)
{
    ...
    if (vkCreateImageView(context.device, &viewInfo, nullptr, &binding.vkImageView) !=
        VK_SUCCESS)
    {

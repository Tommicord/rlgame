    ...
    const glm::vec3& sunDirection, const glm::vec3& targetPosition)
{
  ...
  glm::mat4 lightView = glm::lookAt(shadowCameraPos, targetPosition, glm::vec3(0.0f, 1.0f, 0.0));
    ...
    Game::VulkanContext&                               context)
{
  ...
  shadowBarrier.oldLayout = vk.shadowMapInitialized ? VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL
  ...
  UnitBeginShadowMapRenderPass(
      context.commandBuffers[0], vk.shadowMapRenderPass, vk.shadowMapFramebuffer, 1024, 1024);
  ...
  vkCmdPushConstants(context.commandBuffers[0], vk.shadowPipelineLayout, VK_SHADER_STAGE_VERTEX_BIT,
  ...
  vkCmdPipelineBarrier(context.commandBuffers[0],
      ...
      VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT, 0, 0, nullptr, 0, nullptr, 1, &shadowBarrier);
}

...
void UnitCreateCurvatureComputePipelineLayout(VkDevice device,
    ...
    VkPipelineLayout&                                  pipelineLayout)
{
  ...
  if (vkCreatePipelineLayout(
          device, &curvePipelineLayoutInfo, nullptr, &pipelineLayout) != VK_SUCCESS)
  {
    ...
    VkDevice device, VkPipelineLayout pipelineLayout, VkPipeline& pipeline)
{
  ...
  auto curveComputeShaderModule =
      Providers::ShaderObject::Module(device, curveComputeShaderCode);
  ...
  if (vkCreateComputePipelines(device, VK_NULL_HANDLE, 1, &curveComputePipelineInfo,

    ...
    const glm::vec3& sunDirection, const glm::vec3& targetPosition)
{
  ...
  glm::mat4 lightView =
      glm::lookAt(shadowCameraPos, targetPosition, glm::vec3(0.0f, 1.0f, 0.0f));
    ...
    Main::MainBinding&                                        context)
{
  ...
  lightingData.additionalLights[0].direction =
      glm::normalize(glm::vec3(-0.3f, 0.5f, -0.4f));
  lightingData.additionalLights[0].color = glm::vec3(0.6f, 0.7f, 0.9f);
  lightingData.additionalLights[0].intensity = 2.0f;
  // warm rim light
  lightingData.additionalLights[1].direction =
      glm::normalize(glm::vec3(-0.8f, 0.2f, 0.5f));
  ...
  vkCmdUpdateBuffer(context.commandBuffers[0], vk.placeholderLightingBuffer, 0,
      lightingBlockSize, &lightingData);
  ...
  if (vk.curveComputePipeline != VK_NULL_HANDLE &&
      vk.curveComputePipelineLayout != VK_NULL_HANDLE)
  {
    // Reset counters for curvature compute
    uint32_t zeroCounters[2] = {0, 0};
    vkCmdUpdateBuffer(context.commandBuffers[0], vk.curveCountersBuffer, 0,
        2 * sizeof(uint32_t), zeroCounters);
    ...
    curveResetBarrier.dstAccessMask =
        VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT;

    vkCmdPipelineBarrier(context.commandBuffers[0], VK_PIPELINE_STAGE_TRANSFER_BIT,
        VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, 0, 1, &curveResetBarrier, 0, nullptr, 0,
        nullptr);

    vkCmdBindPipeline(context.commandBuffers[0], VK_PIPELINE_BIND_POINT_COMPUTE,
        vk.curveComputePipeline);
  ...
  if (vk.computePipeline != VK_NULL_HANDLE && vk.computePipelineLayout != VK_NULL_HANDLE)
  {
    ...
    workgroupCount = workgroupCount > 1u ? workgroupCount : 1u;
    ...
    vkCmdPipelineBarrier(context.commandBuffers[0], VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT,
        VK_PIPELINE_STAGE_TRANSFER_BIT, 0, 0, nullptr, 1, &computeToCopyBarrier, 0,
        nullptr);
    ...
    vkCmdFillBuffer(context.commandBuffers[0], vk.indirectDrawBuffer,
        sizeof(uint32_t) * 2, sizeof(uint32_t), 0);
    vkCmdFillBuffer(context.commandBuffers[0], vk.indirectDrawBuffer,
        sizeof(uint32_t) * 3, sizeof(uint32_t), 0);
    vkCmdFillBuffer(context.commandBuffers[0], vk.indirectDrawBuffer,
        sizeof(uint32_t) * 4, sizeof(uint32_t), 0);

...
class LogFormatter
{
  public:
    ...
    static int
    format(char* buffer, size_t bufferSize, const char* fmt, T&& value, Args&&... args) noexcept
    {
      ...
      while (*ptr != '\0' && pos < bufferSize - 1)
      {
        if (*ptr == '%')
        {
          formatValue(buffer, pos, bufferSize, ptr, std::forward<T>(value));
          if constexpr (sizeof...(Args) > 0)
          {
            int remaining =
                format(buffer + pos, bufferSize - pos, ptr, std::forward<Args>(args)...);
            if (remaining >= 0)
              pos += static_cast<size_t>(remaining);
          }
          else
          {
            int remaining = LogFormatter::format(buffer + pos, bufferSize - pos, ptr);
            if (remaining >= 0)
              pos += static_cast<size_t>(remaining);
          }
          break;
  ...
  private:
    ...
    static void formatValue(
        char* buffer, size_t& pos, size_t bufferSize, const char*& format, T&& value) noexcept
    {
      FormatSpec  spec;
      ...
      if (spec.type == FormatType::Array)
      {
        if constexpr (std::is_same_v<std::decay_t<T>, LogArray<float>> ||
                      ...
                      std::is_same_v<std::decay_t<T>, LogArray<unsigned int>>)
        {
    ...
    template <typename T>
    static void
    formatArray(char* buffer, size_t& pos, size_t bufferSize, const LogArray<T>& array) noexcept
    {
      ...
      for (size_t i = 0; i < array.count && pos < bufferSize - 2; i++)
      {
        ...
        if constexpr (std::is_same_v<T, float> || std::is_same_v<T, double>)
        {
          ...
          LogFormatTypes::formatFloat(buffer, pos, bufferSize, static_cast<double>(array.data[i]),
                                      elementSpec);
        }
        else if constexpr (std::is_signed_v<T>)
        {
          ...
          LogFormatTypes::formatint(buffer, pos, bufferSize, static_cast<int64_t>(array.data[i]),
                                    elementSpec);
        }
        else
        {
          ...
          LogFormatTypes::formatUint(buffer, pos, bufferSize, static_cast<uint64_t>(array.data[i]),
                                     elementSpec);
        }
        if (pos >= bufferSize - 1)
          break;
     ...
     * @param value Value to format */
    template <typename T>
    static void formatTyped(char* buffer, size_t& pos, size_t bufferSize, T&& value) noexcept
    {
      FormatSpec spec;
                    ...
                    std::is_same_v<std::decay_t<T>, double>)
      {
        ...
        LogFormatTypes::formatFloat(buffer, pos, bufferSize, static_cast<double>(value), spec);
      }
      else if constexpr (std::is_integral_v<std::decay_t<T>>)
      {
        ...
        {
          spec.isSigned = true;
          LogFormatTypes::formatint(buffer, pos, bufferSize, static_cast<int64_t>(value), spec);
        }
        else
        {
          spec.isSigned = false;
          LogFormatTypes::formatUint(buffer, pos, bufferSize, static_cast<uint64_t>(value), spec);
        }
      }
      else if constexpr (std::is_pointer_v<std::decay_t<T>>)
      {
        if constexpr (std::is_same_v<std::decay_t<T>, const char*>)
        {
          ...
          LogFormatTypes::formatString(buffer, pos, bufferSize, value, spec);
        }
        else
        {
          spec.type = FormatType::Pointer;
          LogFormatTypes::formatPointer(buffer, pos, bufferSize,
      ...
      else
      {
        ...
        spec.type = FormatType::String;
        LogFormatTypes::formatString(buffer, pos, bufferSize, "unknown", spec);
      }
    }
};

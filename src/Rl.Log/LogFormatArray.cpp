        ...
        for (size_t i = 0; i < count && pos < bufferSize - 2; i++)
        {
                ...
                if constexpr (std::is_same_v<T, float> || std::is_same_v<T, double>)
                {
                        LogFormatTypes::formatFloat(buffer, pos, bufferSize,
                                                    static_cast<double>(data[i]), elementSpec);
                }
                else if constexpr (std::is_signed_v<T>)
                {
                        LogFormatTypes::formatint(buffer, pos, bufferSize,
                                                  static_cast<int64_t>(data[i]), elementSpec);
                }
                else
                {
                        LogFormatTypes::formatUint(buffer, pos, bufferSize,
                                                   static_cast<uint64_t>(data[i]), elementSpec);
                }
                                    ...
                                    const FormatSpec& spec) noexcept
{
        ...
        {
        case 'd':
                formatTypedArray(buffer, pos, bufferSize, static_cast<const int32_t*>(data), count,
                                 spec);
                break;
        case 'u':
                formatTypedArray(buffer, pos, bufferSize, static_cast<const uint32_t*>(data), count,
                                 spec);
                break;
        case 'f':
                formatTypedArray(buffer, pos, bufferSize, static_cast<const float*>(data), count,
                                 spec);
                break;
        case 's':
                {
                        ...
                        for (size_t i = 0; i < count && pos < bufferSize - 2; i++)
                        {
                                ...
                                auto* str = strArray[i];
                                if (str != nullptr)
                                        LogFormatTypes::formatString(buffer, pos, bufferSize, str,
                                                                     elementSpec);
                                else
                                        LogFormatTypes::formatString(buffer, pos, bufferSize,
                                                                     "null", elementSpec);
                        }
                }
                break;

...
class Log
{
  public:
     ...
     * @param format Format string
     * @param args Arguments for format string */
    template <typename... Args> static void warning(const char* format, Args&&... args) noexcept
    {
  ...
  private:
     ...
     * @param args Arguments for format string */
    template <typename... Args>
    static void logInternal(LogLevel level, const char* format, Args&&... args) noexcept;
     ...
     * @param args Arguments for format string */
    template <typename... Args>
    static void logWithStackTrace(LogLevel level, const char* format, Args&&... args) noexcept;
...
void Log::logInternal(LogLevel level, const char* format, Args&&... args) noexcept
{
  ...
  int formatted =
      LogFormatter::format(buffer + pos, sizeof(buffer) - pos, format, std::forward<Args>(args)...);
...
void Log::logWithStackTrace(LogLevel level, const char* format, Args&&... args) noexcept
{
  ...
  if (config.enableStackTrace)
  {
    ...
    if (frameCount > 0)
    {
      ...
      LogStackTrace::formatStackTrace(stackBuffer.data(), sizeof(stackBuffer), frames.data(),
                                      frameCount);
      if (stackBuffer[0] != '\0')
      {
        LogConsole* console = getConsole();

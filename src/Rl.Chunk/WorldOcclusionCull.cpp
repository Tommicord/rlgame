...
{
        computeShaderModule = GameShaderLoader::createShaderModule(
            device, WorldOcclusionCullComp_data, WorldOcclusionCullComp_size);
...
{
        WorldOcclusionCullPResource* resource =
            static_cast<WorldOcclusionCullPResource*>(pResource);
        ...
        vkCmdPushConstants(commandBuffer.getCommandBuffer(), pipelineLayout,
                           VK_SHADER_STAGE_COMPUTE_BIT, 0, sizeof(WorldOcclusionCullPushConstants),
                           params);

...
void unpackR8ToFloatSIMD(const uint8_t* pixelData, std::vector<WorldHeightmapData>& output)
{
        ...
        {
                __m128i eBytes =
                    _mm_loadu_si128(reinterpret_cast<const __m128i*>(elevationData + i));
                __m128i tBytes =
                    _mm_loadu_si128(reinterpret_cast<const __m128i*>(temperatureData + i));
                __m128i mBytes =
                    _mm_loadu_si128(reinterpret_cast<const __m128i*>(moistureData + i));

...
template <typename KeyType, typename ValueType, typename HashFunc> class PreHashTable
{
        ...
        private:
                ...
                std::vector<HashTableEntry<KeyType, ValueType>>
                                        entries; /**< Contiguous array of entries */
...
template <typename T, typename BucketType, typename KeyType, typename HashFunc> class PreRegistry
{
        ...
        protected:
                ...
                PreHashTable<KeyType, BucketType, HashFunc>
                                     hashTable; /**< Cache-friendly hash table */
...
void PreHashTable<KeyType, ValueType, HashFunc>::resize(size_t newBucketCount)
{
        ...
        for (size_t i = 0; i < entries.size(); ++i)
        {
                ...
                else
                {
                        ...
                        {
                                size_t probeBucketIdx =
                                    (newBucketIdx + retry) & (newBucketCount - 1);
                                if (newBuckets[probeBucketIdx].count < bucketCapacity)
                                {
                                        newBuckets[probeBucketIdx]
                                            .indices[newBuckets[probeBucketIdx].count++] =
                                            newEntryIdx;
                                        break;
                                }
                        }

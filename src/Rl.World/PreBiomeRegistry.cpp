                                                           ...
                                                           float temperature)
{
  ...
  for (const auto& key : allKeys)
  {
    ...
    if (isBucketNearCondition(key, conditionKey))
    {
      ...
      if (bucket != nullptr)
      {
        biomesInBucket(*bucket, result, elevation, equator, moisture, temperature);
      }
    }
  }
                                      ...
                                      float                   temperature) const
{
  ...
  {
    bool elevationMatch =
        elevation >= biome->getStartElevation() && elevation <= biome->getEndElevation();
    bool moistureMatch =
        moisture >= biome->getStartMoisture() && moisture <= biome->getEndMoisture();
    bool temperatureMatch =
        temperature >= biome->getStartTemperature() && temperature <= biome->getEndTemperature();
    bool equatorMatch = equator >= biome->getStartEquator() && equator <= biome->getEndEquator();

...
{
        std::sort(units.begin(), units.end(), [](const PreUnit* a, const PreUnit* b)
                                                        ...
                                                        float temperature) const
{
        ...
        for (const auto& key : allKeys)
        {
                ...
                if (isBucketNearCondition(key, conditionKey))
                {
                        ...
                        if (bucket != nullptr)
                        {
                                unitsInBucket(*bucket, result, elevation, equator, moisture,
                                              temperature);
                        }
                }
        }
...
PreUnitBucketKey PreUnitRegistry::genBucketKey(const PreUnit& unit) const
{
        ...
        key.elevationRange =
            normalizeToRange(avgElevation, minElevation, maxElevation, _bucketRange);
                                                  ...
                                                  float temperature) const
{
        PreUnitBucketKey key{};
        key.elevationRange =
            normalizeToRange(elevation, minElevation, maxElevation, _bucketRange);

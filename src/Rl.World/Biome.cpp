...
PreBiome::PreBiome(PreBiomeRegister biomeRegister, PreUnitRegistry& unitRegistry) noexcept
    : prop(std::make_unique<Prop>()),
      spawnables(biomeMaxSpawnablesCount),
...
void PreBiome::checkOverlappingEmplace(PreUnit* pUnit)
{
        ...
        bool b2 = rangesOverlap(pUnit->getMoistureStart(), pUnit->getMoistureEnd(),
                                prop->startMoisture, prop->endMoisture);
        bool b3 = rangesOverlap(pUnit->getElevationStart(), pUnit->getElevationEnd(),
                                prop->startElevation, prop->endElevation);
        bool b4 = rangesOverlap(pUnit->getEquatorStart(), pUnit->getEquatorEnd(),
                                prop->startEquator, prop->endEquator);
        if (b1 && b2 && b3 && b4)
        {
...
bool PreBiome::matchesConditions(float temperature, float moisture, float equator,

...
void CompositorManager::createFullscreenQuadPipeline(GameDeviceInstance& device)
{
        ...
        vertexBuffer = GameVulkanBuffer(
            &allocator, bufferSize, VK_BUFFER_USAGE_VERTEX_BUFFER_BIT,
            VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);

        void* data;
        vkMapMemory(device.getDevice(), vertexBuffer.getMemory(), vertexBuffer.getOffset(),
                    bufferSize, 0, &data);
...
void CompositorManager::createDescriptorSets(GameDeviceInstance& device)
{
        ...
        if (vkAllocateDescriptorSets(device.getDevice(), &allocInfo, descriptorSets.data()) !=
            VK_SUCCESS)
        {

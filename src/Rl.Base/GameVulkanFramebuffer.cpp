...
{
  VkResult result = vkCreateFramebuffer(device, createInfo.pCreateInfo, nullptr, &framebuffer);
  if (result != VK_SUCCESS)
  {
    GameError::exitWithError(
        "vkCreateFramebuffer",
        "Failed to create framebuffer (result = " + GameError::vulkanResultToString(result) + ")",

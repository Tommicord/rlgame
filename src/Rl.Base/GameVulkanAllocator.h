...
class GameVulkanMemoryBlock
{
  public:
    ...
    bool
    allocate(VkDeviceSize size, VkDeviceSize alignment, GameVulkanSuballocation& outAllocation);
  ...
  private:
    struct FreeRegion
    {
        VkDeviceSize offset;
        VkDeviceSize size;
        FreeRegion(VkDeviceSize offset, VkDeviceSize size) : offset(offset), size(size)
        {
        }
        ~FreeRegion() = default;
    };
...
{
    static constexpr VkDeviceSize defaultMaxBlockSize = 64000000; // 64 MB max block size

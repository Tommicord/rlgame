                                          ...
                                          GameVulkanSemaphore& initialWaitSemaphore)
{
        ...
        if (computeDispatchCount > 1)
        {
                ...
                std::scoped_lock lock(stateMutex);
                ownedSemaphores.insert(ownedSemaphores.end(),
                                       std::make_move_iterator(chainSemaphores.begin()),
        ...
        for (size_t i = 0; i < computeDispatchCount; ++i)
        {
                ...
                if (i < computeDispatchCount - 1)
                {
                        ...
                        currentWaitSemaphore.setSemaphoreNonOwning(
                            ownedSemaphores[ownedSemaphores.size() - (computeDispatchCount - 1 - i)]
                                .getSemaphore());
                }
                else
                {
    ...
    const GameVulkanSemaphore& newWaitSemaphore)
{
        ...
        if (previousSemaphore != VK_NULL_HANDLE &&
            previousSemaphore != newWaitSemaphore.getSemaphore())
        {
                GameVulkanFence localFence(device, GameVulkanFenceCreateInfo{0});

                submitWait(lastCompletionSemaphore, VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT,
                           localFence);

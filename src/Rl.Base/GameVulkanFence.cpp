    ...
    device(device), fence(VK_NULL_HANDLE)
{
  ...
  if (result != VK_SUCCESS)
  {
    GameError::exitWithError(
        "vkCreateFence",
        "Failed to create fence (result = " + GameError::vulkanResultToString(result) + ")", device,
...
void GameVulkanFence::wait(uint64_t timeout) const
{
  ...
  {
    GameError::exitWithError(
        "Failed to wait for fence (result = " + GameError::vulkanResultToString(result) + ")",
...
void GameVulkanFence::reset()
{
  ...
  {
    GameError::exitWithError(
        "Failed to reset fence (result = " + GameError::vulkanResultToString(result) + ")", device,
        VK_NULL_HANDLE, VK_NULL_HANDLE);
  }
}

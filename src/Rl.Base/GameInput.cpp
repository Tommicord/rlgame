...
template <size_t N> void GameInput<N>::processEvent(const GameInputEvent& event)
{
  ...
  for (size_t i = 0; i < pool->count; ++i)
  {
    ...
    case GameInputEvent::Type::KeyPress:
      receiver->onKeyPress(event.keyCode, event.ctrlPressed, event.shiftPressed, event.altPressed);
      break;
    ...
    case GameInputEvent::Type::KeyRepeat:
      receiver->onKeyRepeat(event.keyCode, event.ctrlPressed, event.shiftPressed, event.altPressed);
      break;
    ...
    case GameInputEvent::Type::MouseScroll:
      receiver->onMouseScroll(event.scrollDeltaX, event.scrollDeltaY, event.mouseX, event.mouseY);
      break;
    default:
      break;
    }
  ...
  case WM_KEYDOWN:
    {
      ...
      event.type    = wasPressed ? GameInputEvent::Type::KeyRepeat : GameInputEvent::Type::KeyPress;
  ...
  case WM_XBUTTONDOWN:
    {
      ...
      event.button =
          (uMsg == WM_XBUTTONDOWN)
              ? ((GET_XBUTTON_WPARAM(wParam) == XBUTTON1) ? GameInputEvent::MouseButton::X1

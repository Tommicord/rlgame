    ...
    device(device), pipeline(VK_NULL_HANDLE), ownsPipeline(true)
{
  ...
  {
    result = vkCreateGraphicsPipelines(device, createInfo.pipelineCache, 1,
                                       createInfo.pGraphicsCreateInfo, nullptr, &pipeline);
  }
  else if (createInfo.pComputeCreateInfo)
  {
    result = vkCreateComputePipelines(device, createInfo.pipelineCache, 1,
  ...
  if (result != VK_SUCCESS)
  {
    GameError::exitWithError(
        "vkCreatePipelines",
        "Failed to create pipeline (result = " + GameError::vulkanResultToString(result) + ")",

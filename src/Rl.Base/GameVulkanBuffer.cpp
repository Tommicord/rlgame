...
void GameVulkanBuffer::copyBufferData(const GameVulkanBuffer& other)
{
        ...
        vkGetBufferMemoryRequirements(allocator->getDevice(), stagingBuffer,
        ...
        void*        srcData;
        vkMapMemory(allocator->getDevice(),
                    dedicated ? other.dedicatedMemory : other.allocation.memory, srcOffset, size, 0,
                    &srcData);
        void* stagingData;
        ...
        void*        dstData;
        vkMapMemory(allocator->getDevice(), dedicated ? dedicatedMemory : allocation.memory,
                    dstOffset, size, 0, &dstData);
        vkMapMemory(allocator->getDevice(), stagingMemory, 0, size, 0, &stagingData);
    ...
    dedicatedBuffer(VK_NULL_HANDLE), dedicatedMemory(VK_NULL_HANDLE)
{
        if (dedicated)
        {
                ...
                if (vkCreateBuffer(allocator->getDevice(), &bufferInfo, nullptr,
                                   &dedicatedBuffer) != VK_SUCCESS)
                {
                        throw std::runtime_error("Failed to create dedicated buffer");
                }

                VkMemoryRequirements memRequirements;
                vkGetBufferMemoryRequirements(allocator->getDevice(), dedicatedBuffer,
                                              &memRequirements);

                VkMemoryAllocateInfo allocInfo{};
                allocInfo.sType           = VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO;
                allocInfo.allocationSize  = memRequirements.size;
                allocInfo.memoryTypeIndex = allocator->findMemoryType(
                    allocator->getPhysicalDevice(), memRequirements, properties);

                if (vkAllocateMemory(allocator->getDevice(), &allocInfo, nullptr,
                                     &dedicatedMemory) != VK_SUCCESS)
                {
                        vkDestroyBuffer(allocator->getDevice(), dedicatedBuffer, nullptr);
                        throw std::runtime_error("Failed to allocate dedicated buffer memory");
                }

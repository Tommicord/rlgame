...
std::string captureBacktraceWindows()
{
  ...
  for (USHORT i = start; i < frames; ++i)
  {
    ...
    {
      oss << "  #" << std::setw(2) << i - start << " " << symbol->Name << " at 0x" << std::hex
    ...
    {
      oss << "  #" << std::setw(2) << i - start << " ??? at 0x" << std::hex << address << std::dec
          << "\n";
    }
...
std::string captureBacktraceUnix()
{
  ...
  if (symbols)
  {
    for (int i = start; i < frames; ++i)
    {
      ...
      {
        std::string mangled   = symbolStr.substr(mangleStart + 1, mangleEnd - mangleStart - 1);
        int         status    = 0;

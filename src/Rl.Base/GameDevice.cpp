...
void GameDeviceInstance::setupDrawables()
{
        ...
        CompositorManager::initialize(gameDevice.device, gameDevice.physicalDevice,
                                      maxFramesInFlight);
...
void GameDeviceInstance::createInstance()
{
        ...
        if (_enableValidationLayers)
        {
                ...
                {
                        if (strcmp(ext.extensionName, VK_EXT_VALIDATION_FEATURES_EXTENSION_NAME) ==
                            0)
                        {
...
SwapChainSupport GameDeviceInstance::querySwapChainSupport(VkPhysicalDevice device)
{
        ...
        vkGetPhysicalDeviceSurfaceCapabilitiesKHR(device, gameDevice.surface,
        ...
        if (presentModeCount != 0)
        {
                ...
                vkGetPhysicalDeviceSurfacePresentModesKHR(
                    device, gameDevice.surface, &presentModeCount, details.presentModes.data());
        }
        return details;
}
...
void GameDeviceInstance::cleanupSyncObjects()
{
        ...
        {
                vkDestroySemaphore(gameDevice.device, gameDevice.imageAvailableSemaphores[i],
                                   nullptr);
        }
        for (size_t i = 0; i < gameDevice.renderFinishedSemaphores.size(); ++i)
        {
                vkDestroySemaphore(gameDevice.device, gameDevice.renderFinishedSemaphores[i],
                                   nullptr);
        }
...
QueueFamilyIndices GameDeviceInstance::findQueueFamilies(VkPhysicalDevice device) const
{
        ...
        for (const auto& queueFamily : queueFamilies)
        {
                ...
                if (!gameDevice.headlessMode)
                {
                        ...
                        vkGetPhysicalDeviceSurfaceSupportKHR(device, i, gameDevice.surface,
                                                             &presentSupport);
...
bool GameDeviceInstance::isDeviceSuitable(VkPhysicalDevice device)
{
        ...
        if (!gameDevice.headlessMode)
        {
                if (extensionsSupported)
                {
                        ...
                        swapChain                         = !swapChainSupport.formats.empty() &&

...
R_CSTL_LogFormatTimestamp (char* buffer, size_t bufferSize)
{
        ...
        SYSTEMTIME st;
        GetLocalTime (&st);
        snprintf (buffer, bufferSize, "%04u-%02u-%02u %02u:%02u:%02u.%03u",
...
R_CSTL_LogCaptureBacktrace (char* buffer, size_t bufferSize)
{
        ...
        for (USHORT i = 0; i < count; ++i)
        {
                ...
                DWORD       lineNo           = 0;
                if (g_log.symInitialized
                    && SymGetLineFromAddr64 (process, address, &lineDisplacement, &lineInfo))
                {
                        file   = lineInfo.FileName ? lineInfo.FileName : "?";
                        lineNo = lineInfo.LineNumber;
                }

                snprintf (line, sizeof (line), "#%u %p %s (%s:%lu)\n", (unsigned)i, frames[i], name, file,
...
R_CSTL_LogWriteToDebugBuffer (const R_CSTL_LogEntry* entry)
{
        ...
        size_t used = 0;
        snprintf (debugBuffer, sizeof (debugBuffer), "[%s][tid=%" PRIu32 "][%-5s] %s",
                  entry->timestamp, entry->threadId, level, entry->message);
...
#endif
        const char* level = R_CSTL_LogLevelName (entry->level);
        fprintf (stderr, "[%s][tid=%" PRIu32 "][%-5s] %s", entry->timestamp, entry->threadId,
                 level, entry->message);
        if (entry->message[0] != 0x00)
        {
                size_t len = strlen (entry->message);
...
R_CSTL_LogInit (void)
{
        ...
        R_CSTL_HeapRegisterAllocation (&g_log, g_log.ring, g_log.capacity * sizeof (R_CSTL_LogEntry*),
...
R_CSTL_LogWriteV (R_CSTL_LogLevel level, const char* fmt, va_list args)
{
        ...
        R_CSTL_HeapRegisterAllocation (&g_log, entry, sizeof (R_CSTL_LogEntry),

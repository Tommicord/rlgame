...
#define R_CSTL_ARRAY_SORT_TYPED(Suffix, Type)                                                                \
    ...
    {                                                                                                        \
        ...
        {                                                                                                    \
            long child = (root << 1) + 1;                                                                    \
            if (child + 1 < end && R_CSTL_ArrayCmp##Suffix##Inline (pBase + child, pBase + child + 1) < 0)   \
                ++child;                                                                                     \
    ...
    {                                                                                                        \
        ...
        void*                      data = pData;                                                             \
        long                       mid = (left + right) >> 1;                                                \
        if (cmp (pBase + mid, pBase + left, data) < 0) R_CSTL_ArraySwap##Suffix (pBase + mid, pBase + left); \
    ...
    {                                                                                                        \
        ...
        {                                                                                                    \
            ...
            const long pivot = R_CSTL_ArrayPartition##Suffix##Cb (pBase, left, right, pCmp, pData);          \
            --depthLimit;                                                                                    \
            if (pivot - left < right - pivot)                                                                \
            {                                                                                                \
                R_CSTL_ArrayIntrosort##Suffix##Cb (pBase, left, pivot - 1, depthLimit, pCmp, pData);         \
                left = pivot + 1;                                                                            \
            }                                                                                                \
            else                                                                                             \
            {                                                                                                \
                R_CSTL_ArrayIntrosort##Suffix##Cb (pBase, pivot + 1, right, depthLimit, pCmp, pData);        \
                right = pivot - 1;                                                                           \
            }                                                                                                \
        }                                                                                                    \
    }                                                                                                        \
...
R_CSTL_ArraySiftDown (const struct R_CSTL_ArraySortCtx* pCtx, long start, long end)
{
    ...
    while (R_CSTL_LIKELY ((root << 1) + 1 <= end))
    {
        ...
        if (R_CSTL_LIKELY (child + 1 < end)
            && cmp (R_CSTL_ArrayElementBytes (pCtx, child), R_CSTL_ArrayElementBytes (pCtx, child + 1), data)
                   < 0)
            ++child;

        if (R_CSTL_LIKELY (
                cmp (R_CSTL_ArrayElementBytes (pCtx, root), R_CSTL_ArrayElementBytes (pCtx, child), data)
                >= 0))
            break;
    ...
    if (R_CSTL_LIKELY (pCtx->elemSize == 4u))
    {
        ...
        for (;;)
        {
            while (R_CSTL_LIKELY (i <= j))
            {
                ...
                {
                    // Fall back to scalar for remaining elements
                    while (i <= j && cmp (R_CSTL_ArrayElementBytes (pCtx, i), pPivot, data) < 0)
                        ++i;
                    break;
                }
            ...
            while (R_CSTL_LIKELY (i <= j))
            {
                ...
                {
                    // Fall back to scalar for remaining elements
                    while (i <= j && cmp (R_CSTL_ArrayElementBytes (pCtx, j), pPivot, data) > 0)
                        --j;
                    break;
                }

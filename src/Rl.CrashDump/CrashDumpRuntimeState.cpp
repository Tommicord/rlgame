...
std::string CrashDumpRuntimeState::collectWindowsModules()
{
        ...
        if (EnumProcessModules(GetCurrentProcess(), hModules, sizeof(hModules), &cbNeeded))
        {
                ...
                for (int i = 0; i < count; i++)
                {
                        ...
                        if (GetModuleFileNameExA(GetCurrentProcess(), hModules[i], moduleName,
                                                 MAX_PATH))
                        {
                                MODULEINFO modInfo;
                                if (GetModuleInformation(GetCurrentProcess(), hModules[i], &modInfo,
                                                         sizeof(modInfo)))
                                {
                                        oss << "  " << moduleName << " (base: 0x" << std::hex
                                            << modInfo.lpBaseOfDll << ", size: " << std::dec
                                            << modInfo.SizeOfImage << " bytes)\n";
                                }
                                else
                                {
                                        oss << "  " << moduleName << "\n";
...
std::string CrashDumpRuntimeState::collectMemoryUsage()
{
                                 ...
                                 sizeof(memCounter)))
        {
                ...
                oss << "  Peak Working Set: " << (memCounter.PeakWorkingSetSize / (1024 * 1024))
                    << " MB\n";
                oss << "  Private Bytes: " << (memCounter.PrivateUsage / (1024 * 1024)) << " MB\n";
                oss << "  Page File Usage: " << (memCounter.PagefileUsage / (1024 * 1024))
                    << " MB\n";
        }
        else
        {

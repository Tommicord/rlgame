                                     ...
                                     VkInstance         instance)
{
            ...
            [&file, &currentSize](const std::string& sectionName, const std::string& content)
        {
                ...
                if (currentSize + section.length() > MAX_CRASH_DUMP_SIZE)
                {
                        ...
                        file << "\n[TRUNCATED: Maximum size of " << MAX_CRASH_DUMP_SIZE
                             << " bytes reached]\n";
        ...
        writeSection("APPLICATION CONTEXT",
                     CrashDumpApplicationContext::collectApplicationContext());
        writeSection("VULKAN CONTEXT", CrashDumpVulkanInfo::collectVulkanContext());

        if (device != VK_NULL_HANDLE && physicalDevice != VK_NULL_HANDLE &&
            instance != VK_NULL_HANDLE)
        {
                writeSection("GPU CRASH DUMP", CrashDumpGPUInfo::collectGPUCrashDump(
                         ...
                         MOVEFILE_REPLACE_EXISTING | MOVEFILE_WRITE_THROUGH))
        {
                ...
                Log::error("Failed to rename crash dump file from %s to %s, error: %lu",
                           tempPath.c_str(), filePath.c_str(), error);
        ...
        {
                Log::error("Failed to rename crash dump file from %s to %s, error: %d",
                           tempPath.c_str(), filePath.c_str(), errno);

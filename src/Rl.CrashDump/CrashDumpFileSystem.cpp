...
bool CrashDumpFileSystem::ensureCrashDumpFolderExists()
{
        ...
        while ((pos = folderPath.find('/', pos)) != std::string::npos)
        {
                ...
                {
                        Log::error("Failed to create intermediate directory: %s, error code: %d",
                                   subPath.c_str(), errno);
                        success = false;
                }
                pos++;
        ...
        {
                Log::error("Failed to create final crash dump folder: %s, error code: %d",
                           folderPath.c_str(), errno);
                success = false;
        }
        return success;
#endif

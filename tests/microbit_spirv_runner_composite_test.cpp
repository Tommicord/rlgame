...
class MicrobitSpirvRunnerCompositeTest : public ::testing::Test {
protected:
    void SetUp() override {
...
TEST_F(MicrobitSpirvRunnerCompositeTest, CompositeConstruct_Vector) {
    const uint32_t spv[] = {
        ...
        0x00050050, 0x00000014, 0x00000012, 0x00000010, 0x00000011,  // OpCompositeConstruct
    ...
    EXPECT_EQ(3u, result->cpuWords[1]);  // Both elements from constant

    Cleanup(exec, rprog, prog, ctx);
}

TEST_F(MicrobitSpirvRunnerCompositeTest, CompositeExtract) {
    const uint32_t spv[] = {
        0x07230203, 0x00010500, 0x000d000b, 0x00000020, 0x00000000,
        0x00020011, 0x00000001,
        0x00030003, 0x0000000a, 0x000001c2,
        ...
        0x00040036, 0x0000000e, 0x0000000d, 0x0000000c,
        0x00040036, 0x0000000f, 0x00000014, 0x0000000c,
        0x00050050, 0x00000014, 0x00000010, 0x00000010, 0x00000011,
        0x00050051, 0x0000000c, 0x00000012, 0x00000005, 0x00000000,
        0x00050053, 0x0000000c, 0x00000013, 0x00000010, 0x00000012,  // OpCompositeExtract
        0x000200f8, 0x0000000f,
    };
...
TEST_F(MicrobitSpirvRunnerCompositeTest, CompositeInsert) {
    const uint32_t spv[] = {
        0x07230203, 0x00010500, 0x000d000b, 0x00000020, 0x00000000,
        0x00020011, 0x00000001,
        0x00030003, 0x0000000a, 0x000001c2,
        ...
        0x00050050, 0x00000014, 0x00000010, 0x00000010, 0x00000011,
        0x00050051, 0x0000000c, 0x00000012, 0x00000005, 0x00000000,
        0x00060052, 0x00000014, 0x00000013, 0x00000010, 0x00000012, 0x00000000,  // OpCompositeInsert
        0x000200f8, 0x0000000f,
    };
    ...
    EXPECT_EQ(2u, result->cpuComponentCount);
    EXPECT_EQ(3u, result->cpuWords[0]);
    EXPECT_EQ(3u, result->cpuWords[1]);

...
CompareRecordKey (const void* pLeft, const void* pRight, void* /*pData*/)
{
  ...
  if (pLeftRecord->key < pRightRecord->key)
    return -1;
  if (pLeftRecord->key > pRightRecord->key)
    return 1;
  return 0;
...
TEST_F (CstlArrayTest, SortU32Elements)
{
  ...
  struct R_CSTL_Array*  pArray = R_CSTL_NewArrayWithData (reinterpret_cast<const uint8_t*> (src), sizeof (src));
  ASSERT_NE (nullptr, pArray);
...
TEST_F (CstlArrayTest, SortWithComparatorContext)
{
  ...
  ASSERT_EQ (0, R_CSTL_ArraySort (pArray, sizeof (uint8_t), CompareU8DescendingWithCtx, (void*)&descending));

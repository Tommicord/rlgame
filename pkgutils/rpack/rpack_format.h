 ...
 */
uint64_t R_RPack_Hash64 (const void* pData, size_t length, uint64_t seed);
 ...
 * @return 64-bit hash value
 */
uint64_t R_RPack_Hash64String (const char* pStr, uint64_t seed);
 ...
 */
int R_RPack_ValidateHeader (const struct R_RPackHeader* pHeader);
 ...
 */
uint64_t R_RPack_GetExpectedFileSize (const struct R_RPackHeader* pHeader);
